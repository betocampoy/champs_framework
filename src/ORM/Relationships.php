...
trait Relationships
{
    ...
    protected function hasOne(string $related_model_class, string $foreign_key = null): ?Model
    {
        ...
        return (new $related_model_class)->findById($this->$foreign_key);
    }
     ...
     * @return \BetoCampoy\ChampsModel\Model|null
     */
    protected function hasManyThrough(string $related_model_class, string $through_model_class, string $through_key = null, string $local_key = null, int $search_id = null, string $columns = null, string $adicional_terms = null, string $adicional_params = null): ?Model
    {
        ...
        if(!$columns){
            $throughModelColumns = $throughModel->getColumns();

            $columns = "m.*";
            foreach ($throughModelColumns = $throughModel->getColumns() as $col){
                $col = $col == "id" ? "j.id as join_id" : "j.{$col}";
                $columns = "{$columns}, {$col}";
            }
        }
        ...
        $model->find("j.{$local_key}=:{$local_key}", "{$local_key}={$this->id}", $columns )
          ->join($through_model_class, "m.id=j.{$through_key}");
