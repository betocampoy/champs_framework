...
//        $this->required = $required;
    ...
    {
        return $this->where("id = :id", "id={$id}")->columns($columns)->fetch();
    }
...
//            $query = $this->queryTransformFromTo("SELECT {$this->entity}.{$key} ".strstr($this->query, " FROM ") . $this->join . $this->terms);
    ...
    public function update(array $data, string $terms = null, string $params = null)
    {
        ...
        try {
            ...
            $stmt = Connect::getInstance($this->database)->prepare("UPDATE " . $this->entity . " SET {$dateSet} $this->terms");
...
//            $this->customSafeOnUpdate();
    ...
    /**
     * To security purpose, this method is used to filter the array data using fillable attribute or defaulf filter
     *
     * @param array $data
     * @return array
     */
    ...
    {
        if(in_array(substr($date_column, strpos($date_column, ".")+1), $this->getColumns())){
